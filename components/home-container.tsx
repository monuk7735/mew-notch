...
export default function HomeContainer({ children }: HomeContainerProps) {
    ...
    return (<>
        ...
        <div className={`${inter.className} flex flex-col h-dvh w-dvw`}>
            {/* Background Layer */}
            <div className="fixed h-dvh w-dvw -z-20 bg-white dark:bg-[#2d3436] transition-colors duration-300" />

            {/* Blurred Video Background */}
            <div className="fixed h-dvh w-dvw -z-10 overflow-hidden" aria-hidden="true">
                <video
                    autoPlay
                    loop
                    muted
                    playsInline
                    className="absolute top-1/2 left-1/2 min-w-full min-h-full w-auto h-auto -translate-x-1/2 -translate-y-1/2 object-cover opacity-30 dark:opacity-20 blur-3xl scale-110"
                >
                    <source src={`${assetPrefix}/preview/main.mp4`} type="video/mp4" />
                </video>
            </div>

            {/* Responsive Banner/Header */}
            <div className="w-full z-9999 text-gray-900 dark:text-white p-4 md:p-8 flex flex-col md:flex-row md:justify-between md:items-center transition-colors duration-300">
                <div className="flex items-center justify-between w-full md:w-auto">
                    ...
                    <div className="flex items-center gap-2 md:hidden">
                        ...
                        <button
                            className="p-2 focus:outline-none relative w-10 h-10 flex items-center justify-center"
                        ...
                        >
                            ...
                            <span className="absolute top-1/2 left-1/2 w-6 h-6 -translate-x-1/2 -translate-y-1/2 flex flex-col justify-center items-center">
                                <span
                                    className={`absolute h-0.5 w-6 bg-gray-900 dark:bg-white rounded transition-all duration-300 translate-y-2 ${navOpen ? 'opacity-0' : 'opacity-100'}`}
                                />
                                <span
                                    className={`absolute h-0.5 w-6 bg-gray-900 dark:bg-white rounded my-1 transition-all duration-300 ${navOpen ? 'rotate-45' : 'rotate-0'}`}
                                />
                                <span
                                    className={`absolute h-0.5 w-6 bg-gray-900 dark:bg-white rounded my-1 transition-all duration-300 ${navOpen ? 'opacity-100 -rotate-45' : 'opacity-0 rotate-0'}`}
                                />
                                <span
                                    className={`absolute h-0.5 w-6 bg-gray-900 dark:bg-white rounded transition-all duration-300 -translate-y-2 ${navOpen ? 'opacity-0' : 'opacity-100'}`}
                                />
                ...
                >
                    <Link
                        href="/features"
                        className={`text-lg font-bold transition-colors duration-200 ${currentPage === "features" ? "underline" : ""}`}
